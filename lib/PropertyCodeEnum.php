<?php

declare(strict_types=1);

namespace Mindbox\Loyalty;

class PropertyCodeEnum
{
    public const PROPERTIES_GROUP_NAME = 'Mindbox';

    public const PROPERTIES_MINDBOX_BONUS = 'MINDBOX_BONUS';
    public const PROPERTIES_MINDBOX_PROMO_CODE = 'MINDBOX_PROMO_CODE';
    public const PROPERTIES_MINDBOX_PROMOCODES = 'MINDBOX_PROMOCODES';
    public const PROPERTIES_MINDBOX_ORDER_ID = 'MINDBOX_ORDER_ID';

    public const BASKET_PROPERTY_CODE = 'MINDBOX_PRICE';
}